  ...
  <div class="flex h-screen bg-white dark:bg-gray-950">
    ...
    <aside
      ...
      <nav class="flex-1 flex flex-col items-center gap-3 w-full px-2">
        <UTooltip v-for="link in topLinks" :key="link.to" :text="link.label" :content="{ side: 'right' }">
          <UButton :to="link.to" :icon="link.icon" color="neutral" variant="ghost"
      ...
      <div class="mt-auto px-2 w-full flex flex-col items-center gap-4">
        <UTooltip text="Gallery" :content="{ side: 'right' }">
          <UButton icon="i-lucide-image" color="neutral" variant="ghost"
        ...
        <UTooltip text="Profile" :content="{ side: 'right' }">
          <div @click="isProfileOpen = true"
    ...
    <!-- Profile Slideover -->
    <USlideover v-model:open="isProfileOpen" side="left" inset :dismissible="false">
      <template #header="{ close }">
        <div class="flex justify-between items-center w-full">
          <span class="font-semibold text-gray-900 dark:text-white text-[17px]">Cài Đặt người dùng</span>
          <UButton icon="i-lucide-x" color="neutral" variant="outline" @click="close" />
        </div>
      </template>
          ...
          <div class="flex flex-col items-center justify-center pt-8 pb-6 relative">
            <div class="relative flex flex-col items-center">
              <div
                ...
                <UIcon name="i-lucide-user" class="w-12 h-12 text-[#004a77] dark:text-blue-300" />
              </div>
            </div>
          </div>
            ...
            <div v-for="item in profileItems" :key="item.title"
              ...
              <div class="flex flex-col">
                ...
                <span v-if="item.description"
                  class="text-[13px] text-gray-500 dark:text-gray-400 mt-0.5 leading-[1.4]">{{ item.description
                  }}</span>
              </div>
            </div>
          </div>
