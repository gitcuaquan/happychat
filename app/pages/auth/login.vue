...
async function onSubmit(event: FormSubmitEvent<z.output<typeof schema>>) {
  ...
  try {
    const result = await $api.auth.login(event.data) as { token?: string, accessToken?: string }

    // Set cookie if token is returned in response body
    if (result && (result.token || result.accessToken)) {
      const token = useCookie('happy_chat_token')
      token.value = result.token || result.accessToken
    }
        ...
        <div class="text-center space-y-4">
          ...
          <div>
            ...
            <p class="text-sm text-gray-500 dark:text-gray-400 mt-2 font-medium">
              Đăng nhập vào Gomchat để tiếp tục
            </p>
          </div>
        </div>
      </template>
      ...
      >
        ...
        >
          <template #hint>
            <NuxtLink
              to="/forgot-password"
              class="text-sm font-semibold text-green-600 hover:text-green-500 transition-colors"
            >Quên mật khẩu?
            </NuxtLink>
          </template>
          <UInput
      ...
      <template #footer>
        <div class="text-center">
          <p class="text-sm text-gray-500 dark:text-gray-400">
            ...
            <NuxtLink
              to="/signup"
              class="font-bold text-green-600 hover:text-green-500 transition-colors ml-1"
            >Đăng ký
              ngay</NuxtLink>
          </p>
        </div>
